...
--------------------------------
1. “今天”主页新增“备忘录”卡片。
2. 自动显示最近更新的 4 条独立备忘录。
3. 每条卡片显示：标题、更新时间和正文摘要。
4. 点击主页中的备忘录，可直接进入“备忘录”页面并打开对应条目继续编辑。
5. 主页提供“查看全部”和“＋ 新建备忘录”快捷按钮。
6. 新建、修改或删除备忘录后，今天主页会即时刷新。
7. 手机端自动改为单列显示，避免卡片过窄。

兼容性
------
- 保留 V1.3.3 的 Credential Isolation 修复。
- 保留 Supabase 多设备同步、手机登录修复、本地缓存、实时计时器、年度科研热力图和项目/论文维度统计。
- localStorage 键仍为 phdTrackerV1。
- 云端表仍为 public.phd_tracker_state。
- 不需要重新创建 Supabase Project、数据表或账号。

V1.3.3 延续修复：Credential Isolation
-------------------------------------
